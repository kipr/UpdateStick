...
md5sum --quiet --strict -c checklist.chk
MD5RESULT=$?
if [[ $MD5RESULT -eq 0 ]];
then
        echo "All update files look good"
else
	echo "ERROR: update file(s) are corrupt"
	exit
fi
...
BOOTCMD="$(dmesg | grep Kernel\ command\ line | grep -q root=/dev/mmcblk0p2)"
greprc=$?
if [[ $greprc -eq 0 ]];
then
...
smart install pkgs/botball-versions-1.0-20.0.cortexa8hf_vfp_neon.rpm --quiet
...
# harrogate
((COUNT++))
echo "$COUNT/$NUM_PKGS  Removing harrogate..."
smart remove harrogate --quiet
rm -rf /harrogate
echo "$COUNT/$NUM_PKGS  Re-installing harrogate..."
smart install pkgs/harrogate-git-84.0.cortexa8hf_vfp_neon.rpm --quiet

# libwallaby
((COUNT++))
echo "$COUNT/$NUM_PKGS  Updating libwallaby..."
smart install pkgs/libwallaby-git-136.0.cortexa8hf_vfp_neon.rpm --quiet

# libwallaby-dev
((COUNT++))
echo "$COUNT/$NUM_PKGS  Updating libwallaby-dev..."
smart install pkgs/libwallaby-dev-git-136.0.cortexa8hf_vfp_neon.rpm --quiet

# wallaby-utility
((COUNT++))
echo "$COUNT/$NUM_PKGS  Updating wallaby-utility..."
smart install pkgs/wallaby-utility-1.0-1.7.0.cortexa8hf_vfp_neon.rpm --quiet
...
smart install pkgs/botui-git-50.0.cortexa8hf_vfp_neon.rpm --quiet
...
# wallaby_upgrade
TARGET=files/wallaby_upgrade
echo "Copying $TARGET"
cp  $TARGET /usr/bin

# py_compile for python programs
TARGET=files/py_compile.py
echo "Copying $TARGET"
cp  $TARGET /usr/lib/python2.7
...
# hdmi.service for hdmi display switching
TARGET=files/hdmi.service
echo "Copying $TARGET"
cp $TARGET /lib/systemd/system/hdmi.service

# 20-display.conf.hdmi for hdmi display settings
TARGET=files/20-display.conf.hdmi
echo "Copying $TARGET"
cp $TARGET /etc/X11/xorg.conf.d/20-display.conf.hdmi

# 20-display.conf.lcd for lcd display settings
TARGET=files/20-display.conf.lcd
echo "Copying $TARGET"
cp $TARGET /etc/X11/xorg.conf.d/20-display.conf.lcd

# config_display.sh for lcd/hdmi detection/switching script
TARGET=files/config_display.sh
echo "Copying $TARGET"
cp $TARGET /etc/X11/xorg.conf.d/config_display.sh

# xserver-nodm.service for node server ENV vars
TARGET=files/xserver-nodm.service
echo "Copying $TARGET"
cp $TARGET /lib/systemd/system/xserver-nodm.service
